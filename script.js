...
function updateProgressBar() {
    ...
    for(let i = 1; i <= currentStep; i++){
        if(formAnswers[`step-${i}`] && !isStepSkipped(i)) {
            answeredQuestions++;
        }
    }
    const totalQuestions = 6 - (formAnswers['step-2'] === 'ja' ? 1 : 0) - (formAnswers['step-2'] === 'nee' ? 1 : 0);
...
function isLastStep() {
    if (currentStep < 4) return false; // Eerste 3 (of 4) stappen zijn nooit de laatste
    let nextVisibleStep = currentStep + 1;
    while(nextVisibleStep <= totalSteps) {
        if (!isStepSkipped(nextVisibleStep)) {
            return false; // Er is nog een zichtbare stap
        }
        nextVisibleStep++;
    }
    return true; // Geen zichtbare stappen meer gevonden
}
...
function nextStep() {
    ...
    if (currentStep === 4) {
        ...
        if (!pi.value || parseInt(pi.value) < 0) { // 0 is toegestaan
