...
let batterijLadingProfiel = [], batterijActieProfiel = []; // Globale profielen voor de indicator
...
function recalculateAndRedraw() {
    const state = { jaarlijksVerbruikKwh: parseInt(document.getElementById('verbruikSlider').value), aantalPanelen: parseInt(document.getElementById('panelenSlider').value) };
...
function renderScenario3Chart(verbruikData, opbrengstData, batterijCapaciteit) {
    ...
    batterijLadingProfiel = []; // Reset globale profielen
    ...
    scenario3Chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                { label: 'Laden Batterij', data: battLaadData, backgroundColor: 'rgba(241, 196, 15, 0.7)' }
            ]
        },
        options: {
    ...
    // Initialiseer indicator op huidige tijd
    updateBatteryIndicator(new Date().getHours(), batterijCapaciteit);
}
